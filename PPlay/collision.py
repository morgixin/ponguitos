...
class Collision:
    ...
    """
    @staticmethod
    def collided_rect(min1, max1, min2, max2):
        x_result = min1.x >= max2.x or max1.x <= min2.x
        y_result = min1.y >= max2.y or max1.y <= min2.y
        return not x_result or not y_result
    ...
    def perfect_collision(gameimage1, gameimage2):
        ...
        return mask_1.overlap(mask_2, (offset_x, offset_y))
